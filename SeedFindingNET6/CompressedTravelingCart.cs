...
namespace SeedFindingNET6
{
    public class CompressedCartItem
    {
        ...
        public int GetCost()
        {
            return Math.Max(
                100 * (1 + ((State >> 11) & 0xF)),
                ObjectInfo.Get(GetId()).Cost * (3 + ((State >> 9) & 0x3))
                );
        }
        public override string ToString()
        {
            return string.Format(
                "{0} x{1} ({2}) 0b{3}",
                ObjectInfo.Get(GetId()), GetQuantity(), GetCost(),
	...
	{        
        public static List<int> ObjectOrdering = new()
            {
                16,
                18,
                20,
        ...
        public static HashSet<int> ValidObjects = new HashSet<int>(ObjectOrdering);
        public MemoryCache<List<CompressedCartItem>> MemoryCache;
        public CompressedTravelingCart()
        {
            MemoryCache = new MemoryCache<List<CompressedCartItem>>(5, 20); //sliding expire, absol expire times
        }

        public static List<CompressedCartItem> GetStock(int seed)
        {
